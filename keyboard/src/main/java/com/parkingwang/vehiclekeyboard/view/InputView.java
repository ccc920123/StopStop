...
public class InputView extends LinearLayout {
    ...
    private ScenClick mCallBack;
    private AsyscHphm mAsysCall;
    /**
     * 扫描号牌事件
     */
    private  final OnClickListener scenClick=new OnClickListener() {
        @Override
        public void onClick(View view) {
            mCallBack=(ScenClick)mContext;
            mCallBack.scenClick(mContext);
        ...
        public void onClick(View v) {
            ...
            if (meta.clickIndex <= meta.numberLength) {
                ...
                for (OnItemSelectedListener listener : mOnItemSelectedListeners) {
                    position = meta.clickIndex;
                    listener.onSelected(meta.clickIndex);
                }
            }else{//点击任何BUTTON 弹出输入框
                setSelectedState(mButtonItems[meta.numberLength]);
                for (OnItemSelectedListener listener : mOnItemSelectedListeners) {
                    position = meta.numberLength;
                    listener.onSelected(meta.numberLength);
                }
            }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mButtonScen= (ImageView) findViewById(R.id.number_8);
        mButtonEndOf6.setOnClickListener(mOnButtonItemClickListener);
        mButtonScen.setOnClickListener(scenClick);
        ...
        // 最后一位默认隐藏
        set8thItemVisibility(true, true);
    }
        ...
        streamOfShownItems()
                ...
                .ifPresent(new Consumer<Button>() {
                    ...
                    public void accept(Button button) {
                        ...
                        ////该处是通过修改了号牌框后马上将事件上传到activity
                        mAsysCall=(AsyscHphm)mContext;
                        mAsysCall.gethphm();
                        //end
